...
def BPMF(ui_csr, T, D, initial_cutoff, lowest_rating, highest_rating, 
    ...
    for t in range(T):
        ...
        for i in range(N):  # loop over the users
            ...
            mu_i_star_part = alpha * mu_i_star_1 + np.dot(Lambda_U, mu_U)  ###CAREFUL!! Multiplication matrix times a row vector!! It should give as an output a row vector as for how it works
        ...
        true_R_pred = U_old.T @ V_old
        print(f"min value: {true_R_pred.min()}, max value: {true_R_pred.max()}")
    
    return U_new.T, V_new.T
